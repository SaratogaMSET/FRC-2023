...
public class PathGenerator {
	...
	public static Path makePath(WaypointSequence waypoints, 
			...
			if(myPath.equals(null)){
				System.out.println( "PathGenerator made a null path");
			}
			return myPath;
	...
	static Trajectory generateFromPath(WaypointSequence path,
		...
		if (path.getNumWaypoints() < 2) {
			System.out.println("Path with less than 2 waypoints. Returning null.");
			return null;
		}
		...
		for (int i = 0; i < splines.length; ++i) {
			...
			if (!Spline.reticulateSplines(path.getWaypoint(i),
				...
				System.out.println("Unable to 'reticulate spline'. Returning null.");
				return null;
			}
		...
		// THIS IS WHERE 2791 started to write code
		...
		//    Trajectory traj = TrajectoryGenerator.generate(config,
		//            TrajectoryGenerator.TrapezoidalStrategy, 0.0, path.getWaypoint(0).theta,
		//            total_distance, 0.0, path.getWaypoint(0).theta);
